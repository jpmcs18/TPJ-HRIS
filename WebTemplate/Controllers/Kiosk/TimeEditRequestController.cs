...
namespace WebTemplate.Controllers.Kiosk
{
    public class TimeEditRequestController : BaseController
    {
        ...
        public ActionResult Index(Index model)
        {
            ...
            model.TimeEditRequests = TimeEditRequestProcess.Instance.Value.GetList(model.Personnel?.ID ?? 0, model.IsExpired, model.IsPending, model.IsApproved, model.IsCancelled, model.LoginDateTime, model.LogoutDateTime, model.Page, model.GridCount, out int PageCount);
        ...
        public ActionResult EditOrNew(long? id, long? personnelid)
        {
            try
            {
                ...
                if ((id ?? 0) > 0)
                    model = TimeEditRequestProcess.Instance.Value.Get(id ?? 0);
        ...
        public ActionResult Save(TimeEditRequest model)
        {
            try
            {
                ...
                model = TimeEditRequestProcess.Instance.Value.CreateOrUpdate(model, User.UserID);
                ModelState.Clear();
                return PartialViewCustom("_TimeEditRequestEdit", model);
            }
            catch (Exception ex)
        ...
        public ActionResult CancelEditing(long? id)
        {
            try
            {
                TimeEditRequest model = TimeEditRequestProcess.Instance.Value.Get(id ?? 0);
                ModelState.Clear();
        ...
        public ActionResult Delete(TimeEditRequest te)
        {
            if (te?.ID != null)
            {
                try
                {
                    TimeEditRequestProcess.Instance.Value.Delete(te);
